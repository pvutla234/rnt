...
const router = useRouter();
  ...
  return (
    <TouchableOpacity className='w-72 mr-2 bg-white rounded-3xl overflow-hidden'
    style={{
        elevation: 4, 
        shadowColor: '#000', 
        ...
        <View className='absolute top-3 left-3 bg-white/90 px-3 py-1 rounded-full'>
            <Text className='text-xs font-semibold text-blue-600 capitalize'>{property.type}</Text>
        </View>
        ...
        <View className='p-4'>
            <Text className='text-base text-gray-800 font-bold mb-1' numberOfLines={1}>{property.title}</Text>

            <View className='flex-row items-center gap-1 mb-3'>
                <Ionicons name='location-outline' size={13} color={"#6b7280"}/>
                <Text className='text-xs text-gray-500' numberOfLines={1}>{property.address}, {property.city}</Text>
            </View>
